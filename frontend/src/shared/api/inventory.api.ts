...
export async function fetchSessionStockAdjustments(sessionId: string): Promise<StockAdjustmentSummary[]> {
  const { data } = await http.get<{ documents: StockAdjustmentSummary[] }>(
    `/inventory/sessions/${sessionId}/stock-adjustments`,
  );
  return data.documents ?? [];
}

export async function fetchStockAdjustment(id: string): Promise<StockAdjustmentDetail> {
  const { data } = await http.get<StockAdjustmentDetail>(`/inventory/stock-adjustments/${id}`);
  return data;
}

export async function fetchGlobalStockAdjustments(page = 1, pageSize = 30): Promise<PaginatedStockAdjustments> {
  const { data } = await http.get<PaginatedStockAdjustments>(
    `/inventory/stock-adjustments?page=${page}&page_size=${pageSize}`,
  );
  return data;
}
