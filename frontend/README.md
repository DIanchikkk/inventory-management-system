...
- Карточка объекта (`/items/:id`) — один QR на публичный URL карточки (origin из `publicOrigin()`).
- Этикетки (`/reports/labels`) — тот же URL `/items/{id}`.
- Инвентаризация — ввод UUID объекта в активной сессии, не отдельный QR.
- Маршрут `/inventory/item/:itemId` оставлен только для редиректа со старых этикеток.
